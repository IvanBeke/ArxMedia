...
6. **Embedded UI type checks without errors** — `docker compose exec ui sh -lc "pnpm typecheck"` exits 0
7. **Embedded UI builds without errors** — `docker compose exec ui sh -lc "pnpm install && pnpm build"` exits 0
