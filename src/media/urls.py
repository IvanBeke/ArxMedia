from django.urls import path

from . import views

urlpatterns = [
    path('genres/', views.genres_list, name='genres_list'),
    path('search/', views.search, name='search'),
    path('trending/', views.trending, name='trending'),
    path('popular/', views.popular, name='popular'),
    path('movies/<int:tmdb_id>/', views.movie_detail, name='movie_detail'),
    path('movies/<int:tmdb_id>/refresh/', views.refresh_movie_metadata, name='refresh_movie_metadata'),
    path('movies/<int:tmdb_id>/credits/', views.movie_credits, name='movie_credits'),
    path('movies/<int:tmdb_id>/external-ids/', views.movie_external_ids, name='movie_external_ids'),
    path('movies/<int:tmdb_id>/recommendations/', views.movie_recommendations, name='movie_recommendations'),
    path('collections/<int:collection_id>/', views.collection_detail, name='collection_detail'),
    path('people/search/', views.people_search, name='people_search'),
    path('people/<int:person_id>/', views.person_detail, name='person_detail'),
    path('people/<int:person_id>/credits/', views.person_credits, name='person_credits'),
    path('people/<int:person_id>/external-ids/', views.person_external_ids, name='person_external_ids'),
    path('tv/<int:tmdb_id>/', views.tv_detail, name='tv_detail'),
    path('tv/<int:tmdb_id>/refresh/', views.refresh_tv_metadata, name='refresh_tv_metadata'),
    path('tv/<int:tmdb_id>/credits/', views.tv_credits, name='tv_credits'),
    path('tv/<int:tmdb_id>/external-ids/', views.tv_external_ids, name='tv_external_ids'),
    path('tv/<int:tmdb_id>/recommendations/', views.tv_recommendations, name='tv_recommendations'),
    path('tv/<int:tmdb_id>/seasons/<int:season_number>/', views.season_detail, name='season_detail'),
    path('tv/<int:tmdb_id>/seasons/<int:season_number>/credits/', views.season_credits, name='season_credits'),
    path('tv/<int:tmdb_id>/seasons/<int:season_number>/episodes/<int:episode_number>/credits/', views.episode_credits, name='episode_credits'),
]
