...
import type { ApiError, CalendarItem, CollectionDetail, Credits, CustomList, DataImportMode, DataTransferFormat, DataTransferJob, DataTransferSource, EpisodeWatchPayload, ExternalIds, Genre, ListItem, ListItemsResponse, LoginPayload, MediaCard, MediaSearchResponse, MediaType, Movie, PaginatedResponse, PersonCombinedCredits, PersonDetail, PersonSearchResponse, ProfileUpdatePayload, QueryParams, RegisterPayload, Rating, Season, SeasonWatchPayload, ShowProgressItem, Tokens, TVShow, User, UserCard, UserProfile, WatchedEpisode, WatchEntry } from '@/types/api'
...
export const mediaAPI = {
  search: (q: string, type: 'movie' | 'tv' | 'multi' = 'multi', page = 1) => api.get<MediaSearchResponse>('/media/search/', { params: { q, type, page } }), searchPeople: (q: string, page = 1) => api.get<PersonSearchResponse>('/media/people/search/', { params: { q, page } }), genres: () => api.get<Genre[]>('/media/genres/'), trending: (type: 'all' | MediaType = 'all', window: 'day' | 'week' = 'week') => api.get<MediaSearchResponse>('/media/trending/', { params: { type, window } }), popular: (type: MediaType = MEDIA_TYPE.MOVIE, page = 1) => api.get<MediaSearchResponse>('/media/popular/', { params: { type, page } }),
  getMovie: (id: string | number, region?: string) => api.get<Movie>('/media/movies/' + id + '/', { params: { region } }), refreshMovie: (id: string | number) => api.post<Movie>('/media/movies/' + id + '/refresh/', {}), getMovieCredits: (id: string | number) => api.get<Credits>('/media/movies/' + id + '/credits/'), getMovieExternalIds: (id: string | number) => api.get<ExternalIds>('/media/movies/' + id + '/external-ids/'), getMovieRecommendations: (id: string | number, page = 1) => api.get<MediaSearchResponse>('/media/movies/' + id + '/recommendations/', { params: { page } }), getCollection: (id: string | number) => api.get<CollectionDetail>('/media/collections/' + id + '/'), getTV: (id: string | number, region?: string) => api.get<TVShow>('/media/tv/' + id + '/', { params: { region } }), refreshTV: (id: string | number) => api.post<TVShow>('/media/tv/' + id + '/refresh/', {}), getTVCredits: (id: string | number) => api.get<Credits>('/media/tv/' + id + '/credits/'), getTVExternalIds: (id: string | number) => api.get<ExternalIds>('/media/tv/' + id + '/external-ids/'), getTVRecommendations: (id: string | number, page = 1) => api.get<MediaSearchResponse>('/media/tv/' + id + '/recommendations/', { params: { page } }),   getSeason: (showId: string | number, season: string | number) => api.get<Season>('/media/tv/' + showId + '/seasons/' + season + '/'), getSeasonCredits: (showId: string | number, season: string | number) => api.get<Credits>('/media/tv/' + showId + '/seasons/' + season + '/credits/'), getEpisodeCredits: (showId: string | number, season: string | number, episode: string | number) => api.get<Credits>('/media/tv/' + showId + '/seasons/' + season + '/episodes/' + episode + '/credits/'), getPerson: (id: string | number) => api.get<PersonDetail>('/media/people/' + id + '/'), getPersonCredits: (id: string | number) => api.get<PersonCombinedCredits>('/media/people/' + id + '/credits/'), getPersonExternalIds: (id: string | number) => api.get<ExternalIds>('/media/people/' + id + '/external-ids/'),
}
...
export const trackingAPI = {
  ...
  importData: (file: File, format: DataTransferFormat = 'json', source: DataTransferSource = 'arxmedia') => { const form = new FormData(); form.append('file', file); const fullUrl = new URL(baseURL + '/tracking/data/import/', window.location.origin); fullUrl.searchParams.set('data_format', format); fullUrl.searchParams.set('source', source); const token = localStorage.getItem('access_token'); return fetch(fullUrl.toString(), { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : {}, body: form }).then((response) => parseResponse<DataTransferJob>(response)) }, exportData: (format: DataTransferFormat = 'json') => api.post<DataTransferJob>('/tracking/data/export/?data_format=' + format, {}), listJobs: () => api.get<PaginatedResponse<DataTransferJob> | DataTransferJob[]>('/tracking/data/jobs/'), getJobStatus: (jobId: string | number) => api.get<DataTransferJob>('/tracking/data/jobs/' + jobId + '/'), confirmJobImport: (jobId: string | number, importMode: DataImportMode) => api.post<DataTransferJob>('/tracking/data/jobs/' + jobId + '/confirm/', { import_mode: importMode }), cancelJobImport: (jobId: string | number) => api.post<DataTransferJob>('/tracking/data/jobs/' + jobId + '/cancel/', {}),
}
