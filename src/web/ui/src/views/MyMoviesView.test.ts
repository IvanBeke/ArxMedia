...
const { getMyMovies } = vi.hoisted(() => ({ getMyMovies: vi.fn() }))
...
function moviesPayload(count: number, rows: number) {
  return {
    count,
...
async function mountView(initialQuery: LocationQueryRaw = {}) {
...
function callPage(mockFn: { mock: { calls: unknown[][] } }, callIndex: number) {
  const params = mockFn.mock.calls[callIndex]?.[0]
  if (!params || typeof params !== 'object') throw new Error('Expected request parameters')
  return (params as Record<string, unknown>).page
}
