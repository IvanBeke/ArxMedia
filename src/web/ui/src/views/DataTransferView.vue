...
const finishedResultSummary = computed(() => {
  ...
  const listsUpdated = Number(finishedReport.value.lists_updated || 0)
  const deletedLists = Number(finishedReport.value.deleted?.lists || 0)
  const listItemsUpdated = Number(finishedReport.value.list_items_updated || 0)
  if (Number(finishedReport.value.lists_created || 0)) parts.push(`${Number(finishedReport.value.lists_created)} lists created`)
  if (listsUpdated) parts.push(`${listsUpdated} lists updated`)
  if (deletedLists) parts.push(`${deletedLists} lists removed`)
  if (Number(finishedReport.value.list_items_created || 0)) parts.push(`${Number(finishedReport.value.list_items_created)} list items added`)
  if (listItemsUpdated) parts.push(`${listItemsUpdated} list items updated`)
