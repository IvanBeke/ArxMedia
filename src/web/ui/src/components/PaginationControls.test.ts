...
function pageNumberButtons(wrapper: VueWrapper) {
  return wrapper
    .findAll('button')
...
function lastTotalPages(wrapper: VueWrapper) {
  ...
  return required(required(events, 'Total pages events').at(-1), 'Total pages event')[0]
}
...
describe('PaginationControls', () => {
  ...
  it('emits go with the clicked page number', async () => {
    ...
    await required(pageTwoButton, 'Page two button').trigger('click')
  ...
  it('suppresses go events for the current page and out-of-range requests', async () => {
    ...
    await required(activePageButton, 'Active page button').trigger('click')

    const nextButton = wrapper.findAll('button').find((button) => button.attributes('aria-label') === 'Go to page 3')
    await required(nextButton, 'Next page button').trigger('click')
