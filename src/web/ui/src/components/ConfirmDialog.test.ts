...
describe('ConfirmDialog', () => {
  ...
  it('emits confirm without closing itself', async () => {
    ...
    await required(wrapper.findAll('button')[1], 'Confirm button').trigger('click')
  ...
  it('closes on cancel', async () => {
    ...
    await required(wrapper.findAll('button')[0], 'Cancel button').trigger('click')
  ...
  it('disables the confirm button while loading', () => {
    ...
    expect(required(buttons[1], 'Confirm button').attributes('disabled')).toBeDefined()
    expect(required(buttons[1], 'Confirm button').text()).toBe('Unwatching...')
  })
})
