...
describe('EpisodeUnwatchDialog', () => {
  ...
  it('unmarks the episode and emits unwatched on confirm', async () => {
    ...
    expect(required(required(wrapper.emitted('unwatched'), 'Unwatched event')[0], 'Unwatched event payload')[0]).toEqual(TARGET)
  ...
  it('stays open without emitting when the API call fails', async () => {
    ...
    vi.mocked(trackingAPI.unmarkEpisodeWatched).mockRejectedValueOnce(new Error('boom'))
